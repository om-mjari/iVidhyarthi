...
router.post("/", async (req, res) => {
  try {
    ...
    if (!Lecturer_Id || Lecturer_Id.trim() === "")
      missingFields.push("Lecturer_Id");
    ...
    const category = await Tbl_CourseCategories.findOne({
    ...
    if (Topics && Array.isArray(Topics) && Topics.length > 0) {
      ...
      const lastSubTopic = await Tbl_CourseSubTopics.findOne().sort({
        SubTopic_Id: -1,
      });
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (Topics && Array.isArray(Topics)) {
      ...
      const existingTopics = await Tbl_CourseTopics.find({
        Course_Id: courseId,
      });
      ...
      if (existingTopicIds.length > 0) {
        await Tbl_CourseSubTopics.deleteMany({
          Topic_Id: { $in: existingTopicIds },
        });
      }
      ...
      if (Topics.length > 0) {
        const lastTopic = await Tbl_CourseTopics.findOne().sort({
          Topic_Id: -1,
        });
        let nextTopicId = lastTopic ? lastTopic.Topic_Id + 1 : 1;
